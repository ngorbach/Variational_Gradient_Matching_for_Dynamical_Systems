...
%
% Example dynamical system used in this code: Lorenz 96 system with a *100 ODEs*, *half* of which are *unobserved*. The ODE parameter is also unobserved.
...
simulation.init_val = zeros(1,simulation.numb_odes);                       % state values at first time point
simulation.ratio_observed = 0.5;                                            % ratio of observed states
...
time.est = 0:0.125:4;                                                      % estimation times
coord_ascent_numb_iter = 10;                                               % number of coordinate ascent iterations
...
for i = 1:coord_ascent_numb_iter
    ...
    state.proxy.mean = proxy_for_ind_states(state.lin_comb,state.proxy.mean,param_proxy_mean',...
        dC_times_invC,coupling_idx.states,mu,inv_sigma,state.obs_idx,clamp_obs_state_to_GP_regression);
end
...
% <html><h4> Proxy for individual states </h4></html>
%
% $\hat{q}(\mathbf{x}_u) \propto \exp\big( ~ E_{Q_{-u}} \ln \mathcal{N}\left(\mathbf{x}_u ; -\mathbf{B}_{u}^+ \mathbf{b}_u, ~\mathbf{B}_u^{+} ~ (\mathbf{A} + \mathbf{I}\gamma) ~ \mathbf{B}_u^{+T} \right) + E_Q \ln \mathcal{N}\left(\mathbf{x}_u ; \boldmath\mu_u(\mathbf{Y}), \Sigma_u \right) \big)$,

function [state_mean,state_inv_cov] = proxy_for_ind_states(lin_comb,state_mean,...
    ode_param,dC_times_invC,coupling_idx,mu,inv_sigma,state_obs_idx,clamp_obs_state_to_GP_regression)
...
for u = state_enumeration
    ...
    if state_obs_idx(u)
        ...
        state_mean(:,u) = state_inv_cov(:,:,u) \ local_mean_sum;
    end
end
...
%%
% <html><h4> Generate Lorenz 96 ODEs </h4></html>
...
path_ode = './Lorenz96_ODEs.txt';                                                   % path to system of ODEs
...
function [state,time,ode,simulation] = generate_ground_truth(time,state,ode,symbols,simulation)

simulation.state_obs_idx = ones(1,simulation.numb_odes); 
idx = randperm(simulation.numb_odes); idx = idx(1:floor(simulation.numb_odes * simulation.ratio_observed));
simulation.state_obs_idx(idx) = 0;
...
OutX_solver = create_Lorenz96(min(time.true),max(time.true),time.true(2)-time.true(1),state.derivative_variance',[simulation.ode_param, length(state_sym)])';ToutX = time.true';
state.true_all=OutX_solver;
try state.true=state.true_all(itrue,:); catch; state.true = state.true_all(end,:); end
...
% State observations
state_obs_variance = simulation.state_obs_variance(state.true(:,logical(simulation.state_obs_idx)));
state.obs = state.true(:,logical(simulation.state_obs_idx)) + sqrt(state_obs_variance) .* randn(size(state.true(:,logical(simulation.state_obs_idx))));
...
state_mat = eye(size(state.true,2)); state_mat(~logical(simulation.state_obs_idx),:) = [];
