...
%
% Example dynamical system used in this code: Lorenz 63 System with the *y-dimension unobserved*. The ODE parameters are also unobserved.
...
path_ode = './Lorenz63_ODEs.txt';                                                   % path to system of ODEs
...
[ToutX,OutX_solver]=ode45(@(t,x) ode_system_mat(x,simulation.ode_param'), time.true, simulation.init_val);
...
h2 = subplot(2,2,1); h2.FontSize = 20; h2.Title.String = 'ODE parameters';
set(gca,'XTick',[1:length(symbols.param)]); set(gca,'XTickLabel',symbols.param);
hold on; drawnow

obs_ind = find(simulation.state_obs_idx); u2=0;
for u = 1:3
    h{u} = subplot(2,2,u+1); cla; plot(time.true,state.true_all(:,u),'LineWidth',2,'Color',[217,95,2]./255); 
...
for u = 1:3
    ...
    if state.obs_idx(u)
        ...
        legend(h{u},{'true','estimated'},'Location','southwest'); 
    end
end
