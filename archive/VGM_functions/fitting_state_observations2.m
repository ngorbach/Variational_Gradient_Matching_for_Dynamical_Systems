...
function [mu_reshaped,inv_sigma_reshaped] = fitting_state_observations2(inv_C,...
    obs_to_state_relation,simulation,symbols,cov_obs,cov_func_obs)
...
% cov_obs = cov_obs + diag(repmat(0.01,size(cov_obs,1),1));
% mu_reshaped = zeros(numb_time_points,numb_states);
% for u = 1:numb_states
%     mu_reshaped(:,u) = cov_func_obs * cov_obs \ simulation.observations{:,simulation.observed_states(u)};
% end
