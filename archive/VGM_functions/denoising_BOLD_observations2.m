...
function [mu,inv_sigma] = denoising_BOLD_observations2(bold_response,inv_C,symbols,simulation,cov_obs,cov_func_obs)
...
cov_obs = cov_obs;% + diag(b(:,1));
mu = zeros(size(cov_obs,1),3);
for u = 1:3
    mu(:,u) = cov_func_obs * (cov_obs)^(-1)* bold_response(:,u));
end
end