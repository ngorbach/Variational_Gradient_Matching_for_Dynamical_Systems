...
\begin{verbatim}
    [state.proxy.mean,state.proxy.inv_cov] = proxy_for_ind_states(state.lin_comb,...
        ...
        mu,inv_sigma,state.obs_idx,A_plus_gamma_inv,opt_settings);
\end{verbatim}
\begin{verbatim}
end
\end{verbatim}
...
\end{verbatim}
        ...
        \color{lightgray} \begin{verbatim}time taken: 57.9286 seconds
\end{verbatim} \color{black}
...
function [state_proxy_mean,state_proxy_inv_cov] = proxy_for_ind_states(lin_comb,...
    ...
    state_obs_idx,A_plus_gamma_inv,opt_settings)
\end{verbatim}
\begin{par}
...
\begin{verbatim}
for u = 1:2
    h_states{u} = subplot(1,3,u+1); cla;
    p.true = plot(time.true,state.true_all(:,u),'LineWidth',2,'Color',[217,95,2]./255);
    hold on;
    p.obs = plot(simulation.time_samp,state.obs(:,u),'*','Color',[217,95,2]./255,...
        'MarkerSize',10);
    h_states{u}.FontSize = 20; h_states{u}.Title.String = symbols.state{u}(2:end-1);
...
for u = 1:2
    if strcmp(plot_type,'final')
        ...
        p.obs = plot(h_states{u},simulation.time_samp,state.obs(:,u),...
