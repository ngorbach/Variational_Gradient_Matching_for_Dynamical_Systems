...
# # Variational Gradient Matching for Dynamical Systems: Lorenz96
...
from Lorenz96_declarations import *
...
#simulation.observed_states = [symbols.state[i] for i in sorted(np.random.randint(0,len(symbols.state),6))]
simulation.observed_states = sym.symbols(['_x_1','_x_3','_x_5','_x_7','_x_9'])
...
time_points.for_estimation = np.arange(0,4,0.2)
...
state_couplings,odes_couplings_to_states = find_state_couplings_in_odes(odes_sym,symbols)
...
#dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_param=[10,.2])
dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_type='RQ',kernel_param=[0.1,0.5,10])
...
inferred_states = [[0 for i in range(len(state_couplings[j])+1)] for j in range(len(state_couplings))]
for obs_state in simulation.observations.columns.values:
    for i in range(len(odes_couplings_to_states)):
        if sym.symbols(obs_state) in odes_couplings_to_states[i]: 
            inferred_states[i][odes_couplings_to_states[i].index(sym.symbols(obs_state))] += 1

#collections.Counter([x for sublist in odes_couplings_to_states for x in sublist])
...
for i in range(20):
    proxy.param = proxy_for_ode_parameters(proxy.state,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,odes_gradient_param,opt_settings.ode_param_constraint,optimizer='minimizer')
    proxy.state = proxy_for_ind_states(i,inferred_states,odes_couplings_to_states,proxy.state,proxy.param,odes,odes_gradient_states,locally_linear_odes,dC_times_inv_C,eps_cov,state_pred_mean,state_pred_inv_cov,simulation.observations,simulation.state,state_couplings,i,clamp_states_to_observation_fit=True,constraints=opt_settings.state_constraint,optimizer='minimizer',fig_shape=fig_shape)
