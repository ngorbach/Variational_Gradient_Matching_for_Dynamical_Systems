...
from DCM_declarations import *
from import_odes_dcm import *
from simulate_state_dynamics_dcm import *
from GP_regression_dcm import *
from rewrite_odes_as_local_linear_combinations_dcm import *
from proxies_for_ode_parameters_and_states_dcm import *
...
#simulation.ode_param = -0.8 + (0.8-(-0.8)) * np.random.rand(11)
#simulation.ode_param[8:] = -1
...
simulation.observed_states = sym.symbols([])
#simulation.observed_states = sym.symbols(['_q1','_q3','_q2','_v1','_v3','_v2','_f1','_f3','_f2','_s1','_s3','_s2','_n1','_n3','_n2'])
...
simulation.final_time_point = 1156.0
...
simulation.observed_time_points = np.arange(0,359*3.22,3.22)
...
opt_settings.ode_param_constraint = 'nonnegative'
...
time_points.for_estimation = np.arange(0,200,2)#np.arange(0,300,5)#np.arange(0,359*3.22,3.22)#np.arange(0,1156,3)#np.arange(0,359*3.22,3.22)
...
odes,odes_sym,ext_input,func,func_sym = import_odes(symbols,odes_path,ext_input_path)
state_couplings = find_state_couplings_in_odes(odes_sym,symbols)
odes_gradient_param,odes_gradient_states,func_gradient = gradient_of_odes(symbols,odes_sym,func_sym)
...
simulation.state,simulation.observations,func_true,func_obs = setup_simulation(simulation,time_points,symbols,odes,ext_input,func,1)
...
#dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_param=[10,20])
dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_type='RQ',kernel_param=[0.1,10,5])
...
locally_linear_odes.ode_param.B,locally_linear_odes.ode_param.b = rewrite_odes_as_linear_combination_in_parameters(odes_sym,symbols.state,symbols.param,symbols.ext_input)
...
locally_linear_odes.state.R,locally_linear_odes.state.r = rewrite_odes_as_linear_combination_in_states(odes_sym,symbols.state,symbols.param,symbols.ext_input,simulation.observed_states,state_couplings)
...
state_pred_mean,state_pred_inv_cov,func_pred_mean = fitting_state_observations(simulation.observations,symbols.state,simulation.SNR,time_points.for_estimation,cov,cov_obs,cov_state_obs,func_obs)
...
dist = cdist(proxy.state.index.values.reshape(-1,1),ext_input[:,0].reshape(-1,1))
row_ind, col_ind = linear_sum_assignment(dist.T)
ext_input2 = ext_input[row_ind,1:]


# In[18]:


proxy.param = proxy_for_ode_parameters(proxy.state,ext_input2,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,odes_gradient_param,opt_settings.ode_param_constraint,optimizer=opt_settings.optimizer)
#proxy.param = pd.DataFrame(np.zeros((11)),columns=['value'],index=map(str,symbols.param)).rename_axis('ODE parameter symbols')
for i in range(10):
    #proxy.param = simulation.ode_param
    proxy.state,proxy.func = proxy_for_ind_states(proxy.state,ext_input2,proxy.param,odes,odes_gradient_states,func,func_gradient,func_true,func_pred_mean,locally_linear_odes,dC_times_inv_C,eps_cov,state_pred_mean,state_pred_inv_cov,simulation.observations,simulation.state,state_couplings,i,clamp_states_to_observation_fit=True,constraints=opt_settings.state_constraint,optimizer='minimizer')
proxy.param = proxy_for_ode_parameters(proxy.state,ext_input2,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,odes_gradient_param,opt_settings.ode_param_constraint,optimizer=opt_settings.optimizer)
...
setup_simulation(simulation_with_est_param,time_points,symbols,odes,ext_input,func,2,simulation.state,simulation.observations,proxy.state);
