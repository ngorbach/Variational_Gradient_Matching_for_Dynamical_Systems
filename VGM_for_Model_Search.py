...
# # Variational Gradient Matching for Dynamical Systems: Model Search
...
# Example dynamical system used in this code: Lorenz attractor system with the x-dimension unobserved. The ODE parameters are also unobserved.
...
from Model_Search_declarations import *
...
#simulation.ode_param = 0.001*np.random.randint(0,2,9)
simulation.ode_param = 0.01*np.random.rand(26)
#simulation.ode_param = 0.006*np.random.randn(9)
simulation.ode_param[0] = -0.01
simulation.ode_param[6] = -0.01
simulation.ode_param[12] = -0.01
simulation.ode_param[18] = -0.01
simulation.ode_param[14] = -0.01

simulation.ode_param = np.squeeze([[-1.48398639e-01, -8.60258104e-01, -1.57816355e-01,
        -9.88361318e-01, -1.77462921e-01, -4.62082598e-02,
        -2.10602273e-02, -4.78782237e-03, -2.80886888e-04,
         5.21792743e-01,  6.78474453e-02,  3.93633417e-01,
         8.84095560e-02, -8.37911083e-02,  2.34894569e-01,
        -6.69160230e-02, -2.03351246e-02, -2.21957755e-03,
         1.70320550e-01, -3.00065317e-01, -5.92636381e-03,
        -1.00827962e-01,  1.89460453e-02,  2.20702392e-01,
        -4.48338436e-02, -1.09771524e-02, -1.07323574e-03,
         8.86410396e-02, -9.66380543e-02, -7.34569320e-02,
         3.68340750e-02,  6.80904618e-02,  1.34136072e-01,
        -9.08363249e-03, -5.35793199e-04,  1.16997743e-04,
        -8.47490356e-02, -6.76109247e-03, -7.79358058e-02,
        -1.00869715e-01, -2.73307817e-03, -9.60949781e-02,
         2.86779516e-02,  8.34718840e-03,  9.94744703e-04]])



#simulation.ode_param = np.squeeze([[-1.48398639e-01, -8.60258104e-01, -1.57816355e-01,
#        -9.88361318e-01, -1.77462921e-01, -4.62082598e-02,
#        -2.10602273e-02, -4.78782237e-03, -0.0001,
#         5.21792743e-01,  6.78474453e-02,  3.93633417e-01,
#         8.84095560e-02, -8.37911083e-02,  2.34894569e-01,
#        -6.69160230e-02, -2.03351246e-02, -2.21957755e-03,
#         0.1, -3.00065317e-01, -5.92636381e-03,
#        -1.00827962e-01,  1.89460453e-02,  2.20702392e-01,
#        -4.48338436e-02, -1.09771524e-02, -1.07323574e-03,
#         8.86410396e-02, -0.3, -7.34569320e-02,
#        3.68340750e-02,  6.80904618e-02,  1.34136072e-01,
#        -9.08363249e-03, -5.35793199e-04,  1.16997743e-04,
#        -8.47490356e-02, -6.76109247e-03, -0.3,
#        -1.00869715e-01, -2.73307817e-03, -9.60949781e-02,
#         2.86779516e-02,  8.34718840e-03,  9.94744703e-04]])

#simulation.ode_param = np.squeeze([[-2., -8.60258104e-01, -1.57816355e-01,
#        -9.88361318e-01, -1.77462921e-01, -4.62082598e-02,
#        -2.10602273e-02, -4.78782237e-03, -2.80886888e-04,
#         5.21792743e-01,  -2,  3.93633417e-01,
#         8.84095560e-02, -8.37911083e-02,  2.34894569e-01,
#        -6.69160230e-02, -2.03351246e-02, -2.21957755e-03,
#         1.70320550e-01, -3.00065317e-01, -2,
#        -1.00827962e-01,  1.89460453e-02,  2.20702392e-01,
#        -4.48338436e-02, -1.09771524e-02, -1.07323574e-03,
#         8.86410396e-02, -9.66380543e-02, -7.34569320e-02,
#         -2.,  6.80904618e-02,  1.34136072e-01,
#        -9.08363249e-03, -5.35793199e-04,  1.16997743e-04,
#        -8.47490356e-02, -6.76109247e-03, -7.79358058e-02,
#        -1.00869715e-01, -3, -9.60949781e-02,
#         2.86779516e-02,  8.34718840e-03,  9.94744703e-04]])
...
simulation.observed_states = sym.symbols(['_x','_z','_w','_v'])
...
# Input a row vector of postive real numbers in ascending order:

# In[6]:


#simulation.observed_time_points = np.array([0,1,2,4,5,7,10,15,20,30,40,50,60,80,99])
simulation.observed_time_points = np.array([0.,.5,1.,1.5,2.,2.5,3.,3.5,4.,8.,8.5,9.,9.5,10.0,10.5,11.,11.5,12.,16.,17.,18.,19.,20.])#np.arange(0.,20.,1.)
...
opt_settings.ode_param_constraint = 'shrinkage'
...
opt_settings.state_constraint = None
...
#time_points.for_estimation = np.arange(0,99,0.5)
...
opt_settings.optimizer = 'minimizer'
...
#   \dot{x} &= \sigma ~ (y-x)\\
#   \dot{y} &= -x ~ z + \rho ~ x -y \\
#   \dot{z} &= x ~ y - \alpha ~ z \qquad \qquad \qquad (1)
# \end{align}
...
odes_gradient_param,odes_gradient_states = gradient_of_odes(symbols,odes_sym)
...
#dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_type='rbf',kernel_param=[10.,5.])
dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_type='RQ',kernel_param=[0.1,5,1])
...
if opt_settings.optimizer == 'analytical':
...
locally_linear_odes.state.R,locally_linear_odes.state.r = rewrite_odes_as_linear_combination_in_states(odes,symbols.state,symbols.param,simulation.observed_states,state_couplings)
...
proxy.param = pd.DataFrame(0.1*np.ones(len(symbols.param)),columns=['value'],index=map(str,symbols.param)).rename_axis('ODE parameter symbols')
for i in range(10):
    proxy.param = proxy_for_ode_parameters(proxy.state,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,odes_gradient_param,opt_settings.ode_param_constraint,opt_settings.optimizer,fig_shape=(20,5),init=proxy.param.values)
    proxy.state = proxy_for_ind_states(proxy.state,proxy.param,odes,odes_gradient_states,locally_linear_odes,dC_times_inv_C,eps_cov,state_pred_mean,state_pred_inv_cov,simulation.observations,simulation.state,state_couplings,False,opt_settings.state_constraint,optimizer='minimizer',fig_shape=fig_shape)
...
simulation.initial_states = proxy.state.values[0,:]
setup_simulation(simulation_with_est_param,time_points,symbols,odes,fig_shape,2,simulation.state,simulation.observations,proxy.state);
