...
simulation.state_obs_variance = @(mean)(bsxfun(@times,[2,2],ones(size(mean))));% observation noise
simulation.ode_param = [10,28,2.66667];                                    % true ODE parameters
...
simulation.init_val = -7.*ones(1,3);                                       % state values at first time point
simulation.state_obs_idx = [1,0,1];                                        % indices of states that are directly observed (Boolean)
...
%%
% Error variance on state derivatives $\gamma$:
state.derivative_variance = [6,6,6];                                       % gamma for gradient matching model
%%
...
%%
% ODE parameters $\theta$:
symbols.param = {'[\sigma]','[\rho]','[\beta]'};                           % symbols of parameters in 'ODEs.txt' file
...
function [param_proxy_mean,param_proxy_inv_cov] = proxy_for_ode_parameters(state_proxy_mean,dC_times_invC,lin_comb,symbols)
...
path_ode = './Lorenz_attractor_ODEs.txt';                                                   % path to system of ODEs
...
h_param = subplot(2,2,1); h_param.FontSize = 20; h_param.Title.String = 'ODE parameters';
set(gca,'XTick',[1:length(symbols.param)]); set(gca,'XTickLabel',symbols.param);
hold on; drawnow

obs_ind = find(simulation.state_obs_idx); u2=0;
for u = 1:3
    h_states{u} = subplot(2,2,u+1); cla; plot(time.true,state.true_all(:,u),'LineWidth',2,'Color',[217,95,2]./255); 
...
for u = 1:3
    ...
    if state.obs_idx(u)
        legend(h_states{u},{'true','observed','estimated'},'Location','southwest'); 
    else
        legend(h_states{u},{'true','estimated'},'Location','southwest'); 
    end
end
...
legend(h_param,{'true','estimated'},'Location','northwest');
drawnow
