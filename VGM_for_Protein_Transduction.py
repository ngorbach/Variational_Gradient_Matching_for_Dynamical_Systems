...
# # Variational Gradient Matching for Dynamical Systems: Protein Transduction
...
from Protein_Transduction_declarations import *
...
simulation.ode_param = [0.07,0.6,0.05,0.3,0.017,0.3]
...
simulation.observed_states = sym.symbols(['_dS','_R','_R_pp'])
...
simulation.final_time_point = 100.0
...
time_points.for_estimation = np.arange(0,99,1)#np.array([0,1,2,4,5,7,10,15,20,30,40,50,60,80,99])#np.arange(0,99,1)
...
#dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_type='sigmoid',kernel_param=[2.25,22.50,2.25])
dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_type='RQ',kernel_param=[0.1,20,1])
...
state_pred_mean,state_pred_inv_cov = fitting_state_observations(simulation.observations,symbols.state,simulation.SNR,time_points.for_estimation,cov,cov_obs,cov_state_obs,fig_shape)
...
for i in range(10):
    proxy.param = proxy_for_ode_parameters(proxy.state,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,odes_gradient_param,opt_settings.ode_param_constraint,opt_settings.optimizer)
    proxy.state = proxy_for_ind_states(proxy.state,proxy.param,odes,odes_gradient_states,locally_linear_odes,dC_times_inv_C,eps_cov,state_pred_mean,state_pred_inv_cov,simulation.observations,simulation.state,state_couplings,True,opt_settings.state_constraint,optimizer='minimizer',fig_shape=fig_shape)
...
simulation.state2 = simulation.state
