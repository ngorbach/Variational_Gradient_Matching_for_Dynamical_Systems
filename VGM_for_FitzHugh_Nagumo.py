...
# # Variational Gradient Matching for Dynamical Systems: FitzHugh_Nagumo
...
from FitzHugh_Nagumo_declarations import *
...
simulation.ode_param = [0.5,0.7,0.8,12.5]
# ODE parameters shown on the Wikipedia page: https://en.wikipedia.org/wiki/FitzHugh–Nagumo_model
...
simulation.observed_states = sym.symbols(['w','v'])
...
simulation.observed_time_points = np.arange(0,99.0,5)
...
time_points.for_estimation = np.arange(0,99.0,1)
...
simulation.state,simulation.observations = setup_simulation(simulation,time_points,symbols,odes,1)
...
dC_times_inv_C,eps_cov,cov,cov_obs,cov_state_obs = kernel_function(time_points.for_estimation,simulation.observations.index,kernel_param=[2,10])
...
state_pred_mean,state_pred_inv_cov = fitting_state_observations(simulation.observations,symbols.state,simulation.obs_variance,time_points.for_estimation,cov,cov_obs,cov_state_obs)
...
for i in range(10):
    proxy.param = proxy_for_ode_parameters(proxy.state,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,opt_settings.ode_param_constraint,optimizer='minimizer')
    proxy.state = proxy_for_ind_states(proxy.state,proxy.param,odes,locally_linear_odes,dC_times_inv_C,eps_cov,state_pred_mean,state_pred_inv_cov,simulation.observations,simulation.state,state_couplings,True,opt_settings.state_constraint,optimizer='minimizer')
