...
for i in range(3):
    proxy.param = proxy_for_ode_parameters(proxy.state,locally_linear_odes,dC_times_inv_C,eps_cov,symbols.param,simulation.ode_param,odes,opt_settings.ode_param_constraint,optimizer='analytical')
    proxy.state = proxy_for_ind_states(proxy.state,proxy.param,odes,locally_linear_odes,dC_times_inv_C,eps_cov,state_pred_mean,state_pred_inv_cov,simulation.observations,simulation.state,state_couplings,True,opt_settings.state_constraint)
